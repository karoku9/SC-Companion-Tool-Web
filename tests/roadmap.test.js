...
function numericVersion(version) { const [major, minor = '0'] = version.split('.').map(Number); return major * 1000 + minor; }
...
test('v0.17 hardens the delivered interstellar interface', () => {
  ...
  assert.ok(current.changes.some((change) => /viewport/i.test(change)));

...
function defaultState() {
  ...
  return {
    ...
    activeRoute: { routeId: plannedRoute.id, status: 'active', stepIndex: Math.min(2, plannedRoute.steps.length - 1), steps: copy(plannedRoute.steps), metrics: copy(plannedRoute.metrics), lotIds: copy(plannedRoute.lotIds), stopLabels: copy(plannedRoute.stopLabels), customLocations: copy(plannedRoute.customLocations), shipSnapshot: { fleetEntryId: fleet[0].id, shipId: DATA.ships.find(ship => ship.id === fleet[0].shipId).id, label: fleetShipLabel(fleet[0], DATA.ships.find(ship => ship.id === fleet[0].shipId)), model: DATA.ships.find(ship => ship.id === fleet[0].shipId).name, capacity: DATA.ships.find(ship => ship.id === fleet[0].shipId).capacity }, startingLocationSnapshot: { id: 'everus', name: fixedLocationById('everus').name }, manifest: missions.flatMap(mission => mission.cargo.map(lot => ({ ...copy(lot), missionReference: mission.reference, missionTitle: mission.title }))), manualCorrections: {} },
...
function loadState() {
  ...
  try {
    ...
    if (merged.activeRoute?.steps?.length && !merged.activeRoute.shipSnapshot) {
      const entry = merged.fleet.find(item => item.id === merged.selectedShipId) || merged.fleet[0]; const ship = DATA.ships.find(item => item.id === entry.shipId); const firstStep = merged.activeRoute.steps[0];
      merged.activeRoute.shipSnapshot = { fleetEntryId: entry.id, shipId: ship.id, label: fleetShipLabel(entry, ship), model: ship.name, capacity: ship.capacity };
      merged.activeRoute.startingLocationSnapshot = { id: firstStep.locationId || null, name: firstStep.location || 'Unknown' };
    }
...
function fleetShipLabel(entry, ship = DATA.ships.find(item => item.id === entry?.shipId)) { return entry?.nickname ? `${entry.nickname} — ${ship.name}` : ship?.name || 'Unknown ship'; }
...
function activeRouteIsRunning() { return state.activeRoute?.status === 'active'; }
function plannerChangeNotice(detail = '') { const safety = 'The current active route will remain unchanged. Recalculate to apply Planner changes.'; return activeRouteIsRunning() ? `${detail ? `${detail} ` : ''}${safety}` : detail; }
...
function routeMapMarkup(compact = false) {
  ...
  return `<div class="map-chassis"><div class="map-bezel"><span>NAV / STANTON</span><span>LOCAL ESTIMATE · NOT IN-GAME ROUTER</span></div><div class="map-canvas${compact ? ' hauling-map' : ''}">
    ...
    ${nodes.map(location => { const used = routeLocationNames.has(location.name); const isCurrent = location.name === current.name; const isNext = location.name === nextName && showsTravel; const completed = completedNames.has(location.name) && !isCurrent && !isNext; const remaining = used && !completed && !isCurrent && !isNext; const selected = location.id === state.ui.selectedLocationId; return `<button class="map-node${selected ? ' is-selected' : ''}${used ? ' is-route-stop' : ''}${remaining ? ' is-remaining' : ''}${completed ? ' is-completed' : ''}${isCurrent ? ' is-current' : ''}${isNext ? ' is-next' : ''}" type="button" data-location="${location.id}" style="left:${location.x}%;top:${location.y}%"><i></i><strong>${markupSafe(location.name)}</strong><small>${markupSafe(location.type)}</small></button>`; }).join('')}
...
function renderPage() {
  ...
  const ship = selectedShip(); const entry = selectedFleetEntry(); byId('headerShip').textContent = fleetShipLabel(entry, ship); byId('headerShipCapacity').textContent = `${money(ship.capacity)} SCU`; byId('headerTheme').textContent = `${resolvedTheme().toUpperCase()} SYS`; byId('headerLocation').textContent = activeLocationLabel();
...
function requestCalculateRoute() { if (!state.missions.length) return showToast('No missions to route', 'Add or import at least one mission.'); if (activeRouteIsRunning()) openConfirm({ title: 'Recalculate Planner route?', message: 'The current active route will remain unchanged. Recalculate to apply Planner changes.', actionLabel: 'Recalculate draft', tone: 'primary', action: 'calculate-route' }); else calculateRouteNow(); }
function startRouteNow() { mutate(value => { const route = value.plannedRoute; const entry = selectedFleetEntry(); const ship = selectedShip(); const start = selectedStart(); value.activeRoute = { routeId: route.id, status: 'active', stepIndex: 0, steps: copy(route.steps), metrics: copy(route.metrics), lotIds: copy(route.lotIds), stopLabels: copy(route.stopLabels), customLocations: copy(route.customLocations), shipSnapshot: { fleetEntryId: entry.id, shipId: ship.id, label: fleetShipLabel(entry, ship), model: ship.name, capacity: ship.capacity }, startingLocationSnapshot: { id: start.id, name: start.name }, manifest: value.missions.flatMap(mission => mission.cargo.filter(lot => route.lotIds.includes(lot.id)).map(lot => ({ ...copy(lot), missionReference: mission.reference, missionTitle: mission.title }))), manualCorrections: {} }; value.ui.page = 'active'; }); showToast('Route started', `${state.activeRoute.steps.length} generated steps are now active.`); }
function routeStartContext() { return { overBy: totalScu() - selectedShip().capacity, replacingActive: activeRouteIsRunning() }; }
function requestStartRoute() { if (!state.plannedRoute?.steps?.length) return showToast('No route ready', 'Calculate a route first.'); const context = routeStartContext(); const replacement = context.replacingActive ? ' This will end and replace the current active route.' : ''; if (context.overBy > 0) return openConfirm({ title: 'Start overloaded route?', message: `Planned cargo exceeds ${selectedShip().capacity} SCU by ${context.overBy} SCU.${replacement} Confirm that you want to start this route anyway.`, actionLabel: 'Start overloaded route', tone: 'primary', action: 'start-route' }); if (context.replacingActive) return openConfirm({ title: 'Replace active route?', message: 'This will end and replace the current active route.', actionLabel: 'Replace and start', tone: 'primary', action: 'start-route' }); startRouteNow(); }
function performConfirmed(config) {
  if (config.action === 'delete-mission') { mutate(value => { value.missions = value.missions.filter(mission => mission.id !== config.id); value.plannedRoute = null; value.ui.openMissionMenu = null; }); showToast('Mission deleted', plannerChangeNotice('The mission and its cargo lots were removed.')); }
  ...
  if (config.action === 'set-start') { mutate(value => { value.startingLocationId = config.id; value.preferences.defaultStartingLocationId = config.id; value.plannedRoute = null; value.ui.page = 'planner'; }); showToast('Starting location updated', plannerChangeNotice(fixedLocationById(config.id).name)); }
...
function handleClick(event) {
  ...
  const duplicate = event.target.closest('[data-duplicate-mission]'); if (duplicate) { const source = state.missions.find(item => item.id === duplicate.dataset.duplicateMission); const missionId = makeId('mission'); const clone = { ...copy(source), id: missionId, title: `${source.title} copy`, reference: source.reference ? `${source.reference}-COPY` : 'COPY', cargo: source.cargo.map(lot => ({ ...lot, id: makeId('lot'), missionId })) }; mutate(value => { value.missions.push(clone); value.plannedRoute = null; value.ui.openMissionMenu = null; }); showToast('Mission duplicated', plannerChangeNotice('Cargo-lot identities were regenerated.')); return; }
  ...
  const selectShip = event.target.closest('[data-select-ship]'); if (selectShip) { mutate(value => { value.selectedShipId = selectShip.dataset.selectShip; value.plannedRoute = null; }); showToast('Planner ship selected', plannerChangeNotice(`${fleetShipLabel(selectedFleetEntry(), selectedShip())} · ${selectedShip().capacity} SCU`)); return; }
  const removeShip = event.target.closest('[data-remove-ship]'); if (removeShip) { const entry = state.fleet.find(item => item.id === removeShip.dataset.removeShip); openConfirm({ title: 'Remove saved ship?', message: `Remove ${fleetShipLabel(entry, DATA.ships.find(ship => ship.id === entry.shipId))} from the local fleet?`, actionLabel: 'Remove ship', action: 'remove-ship', id: entry.id }); return; }
...
document.addEventListener('change', event => {
  if (event.target.id === 'plannerShip' || event.target.id === 'haulingShip') { mutate(value => { value.selectedShipId = event.target.value; value.plannedRoute = null; }); showToast('Planner ship selected', plannerChangeNotice(`${fleetShipLabel(selectedFleetEntry(), selectedShip())} · ${selectedShip().capacity} SCU`)); }
  if (event.target.id === 'plannerStart') { mutate(value => { value.startingLocationId = event.target.value; value.plannedRoute = null; }); showToast('Starting location updated', plannerChangeNotice(selectedStart().name)); }
  ...
  if (event.target.id === 'defaultShipSetting') mutate(value => { value.preferences.defaultShipCatalogId = event.target.value; let entry = value.fleet.find(item => item.shipId === event.target.value); if (!entry) { entry = fleetEntry(event.target.value, ''); value.fleet.push(entry); } value.selectedShipId = entry.id; value.plannedRoute = null; });
  if (event.target.id === 'defaultStartSetting') { mutate(value => { value.preferences.defaultStartingLocationId = event.target.value; value.startingLocationId = event.target.value; value.plannedRoute = null; }); showToast('Starting location updated', plannerChangeNotice(selectedStart().name)); }

...
(function exposeMissionValidation(root) {
  ...
  function tokenizeCargoDetailed(value) {
    ...
    return freeze({ text, items, remainder, confidence: items.length && !remainder && items.every((item) => item.scu > 0) ? 1 : items.length ? 0.62 : 0 });
  }
  ...
  function inspectLocations(rawName, locationModel, options, entryKey) {
    ...
    locations.forEach((location, index) => {
      ...
      if (confirmed && confirmed === normalize(location.raw)) locations[index] = { ...location, status: 'custom-confirmed', confidence: 0.55, id: `custom-${slug(location.raw) || `${entryKey}-${index}`}` };
    });
    ...
    return freeze({ status: worst.status, confidence: locations.length ? Math.min(...locations.map((item) => item.confidence)) : 0, id: locations.length === 1 ? locations[0].id : null, label: locations.map((item) => item.label).join(' + '), raw: String(rawName ?? '').trim(), candidates: locations.flatMap((item) => item.candidates ?? []), locations });
  }
  ...
  function inspectMissionText(text, locationModel, options = {}) {
    ...
    rawLines.forEach((rawLine, zeroIndex) => {
      ...
      if (!exactAction && !looksObjective) {
        const title = numberedMission ? (numberedMission[2].trim() || `Mission ${numberedMission[1] || missionDrafts.length + 1}`) : line;
        startMission(title, lineNumber, Boolean(numberedMission));
        actionIndex = 0;
        return;
      }
      ...
      mission.entries.forEach((entry) => {
        ...
        entry.cargo.items.forEach((deliveryItem) => {
          ...
          mission.pickupPools.filter((pool) => normalize(pool.commodity) === normalize(deliveryItem.commodity) && pool.remaining > 0).forEach((pool) => {
            ...
            mission.cargoLots.push({ id: `${mission.key}-lot-${mission.cargoLots.length + 1}`, commodity: pool.commodity, scu: allocated, pickupType: pool.pickupType, pickupLocationId: firstPickup.id, pickupLocationLabel: pool.pickupLocations.map((item) => item.label).join(' + '), pickupLocations: pool.pickupLocations, sharedPickupTotal: pool.pickupLocations.length > 1, deliveryLocationId: resolvedLocations[0].id, deliveryLocationLabel: resolvedLocations[0].label, confidence: Math.min(pool.sourceEntry.confidence, entry.confidence), source: { missionTitleLine: mission.titleLine, pickupLine: pool.sourceEntry.line, deliveryLine: entry.line, pickupText: pool.sourceEntry.raw, deliveryText: entry.raw, sharedPickupTotal: pool.pickupLocations.length > 1 } });
    ...
    const missions = missionDrafts.filter((mission) => mission.title && mission.cargoLots.length).map((mission, missionIndex) => ({ id: `mission-${missionIndex + 1}-${slug(mission.title)}`, title: mission.title, contractor: mission.metadata.contractor ?? null, rewardAuec: mission.metadata.rewardAuec ?? null, category: 'cargo', confidence: mission.entries.length ? Math.round(Math.min(...mission.entries.map((entry) => entry.confidence)) * 100) : 0, source: { originalTitle: mission.title, titleLine: mission.titleLine, originalText, entryKeys: mission.entries.map((entry) => entry.key), contractor: mission.metadata.contractor ?? null, rewardAuec: mission.metadata.rewardAuec ?? null }, cargoLots: mission.cargoLots.map((lot, lotIndex) => ({ ...lot, id: `mission-${missionIndex + 1}-${slug(mission.title)}-lot-${lotIndex + 1}` })) }));
    const ready = blockingIssues.length === 0 && missions.length > 0;
    return freeze({ version: 2, originalText, entries, missions, issues: sortedIssues, blockingIssues, warnings, ready, status: blockingIssues.length ? 'blocked' : warnings.length ? 'review' : 'ready', confidence: Math.max(0, Math.min(100, confidence)), summary: { missionCount: missionDrafts.length, cargoLotCount: missions.reduce((sum, mission) => sum + mission.cargoLots.length, 0), blockerCount: blockingIssues.length, warningCount: warnings.length, exactLocationCount: actionEntries.filter((entry) => entry.location.status === 'exact').length, customLocationCount: actionEntries.filter((entry) => entry.location.status === 'custom-confirmed').length } });
  }

  function serializeReview(reviewMissions) {
    return (reviewMissions ?? []).map((mission) => {
      const title = String(mission.title ?? '').trim();
      const objectives = (mission.objectives ?? []).map((objective) => `${ACTIONS.includes(normalize(objective.action)) ? normalize(objective.action) : String(objective.action ?? '').trim()} ${String(objective.location ?? '').trim()} ${String(objective.cargo ?? '').trim()}`.trim());
      return [title, ...objectives].join('\n');
    }).join('\n\n');
  }

  function snapshot(report, sourceText, reviewedText) {
    return freeze({ version: report.version, sourceText: String(sourceText ?? report.originalText), reviewedText: String(reviewedText ?? report.originalText), status: report.status, confidence: report.confidence, summary: report.summary, issues: report.issues.map((item) => ({ severity: item.severity, code: item.code, line: item.line, entryKey: item.entryKey, field: item.field, message: item.message })), reviewedAt: new Date().toISOString() });
  }

  const api = freeze({ ACTIONS, inspectMissionText, tokenizeCargoDetailed, serializeReview, snapshot });
